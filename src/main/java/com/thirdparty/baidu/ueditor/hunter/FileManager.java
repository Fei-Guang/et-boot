package com.thirdparty.baidu.ueditor.hunter;
...
import com.thirdparty.baidu.ueditor.define.MultiState;
	...
	public State listFile(int index) {
		...
		Collections.sort(fs, new Comparator<File>() {
			...
			public int compare(File o1, File o2) {
				...
				long r = o2.lastModified() - o1.lastModified();
				if (r > 0) {
					return 1;
				} else if (r < 0) {
					return -1;
		...
		if (index < 0 || index > fs.size()) {
			...
			Object[] fileList = Arrays.copyOfRange(fs.toArray(), index, index
					+ this.count);
			state = this.getState(fileList);
		}

		state.putInfo("start", index);
		state.putInfo("total", fs.size());
