...
<div id="bigMenuContainer">
	...
	<div id="menu-container">
						...
						<c:choose>
							<c:when test="${bird:length(element.name) > 14}">
								<span title="${element.name}"
									onclick="loadBillItem(${element.subprojectid},0);">${bird:substring(element.name, 14)}...</span>
							</c:when>
							<c:otherwise>
								<span title="${element.name}"
									onclick="loadBillItem(${element.subprojectid},0);">${element.name}</span>
							</c:otherwise>
						</c:choose>
					</div>
							...
							<c:choose>
								<c:when test="${bird:length(branchElement.name) > 12}">
									<span title="${branchElement.name}"
										onclick="loadBillItem(${branchElement.elementid},1);">${bird:substring(branchElement.name, 12)}...</span>
								</c:when>
								<c:otherwise>
									<span title="${branchElement.name}"
										onclick="loadBillItem(${branchElement.elementid},1);">${branchElement.name}</span>
								</c:otherwise>
							</c:choose>
						</div>
								...
								<c:choose>
									<c:when test="${bird:length(leafElement.name) > 10}">
										<span title="${leafElement.name}"
											onclick="loadBillItem(${leafElement.elementid},2);">${bird:substring(leafElement.name, 10)}...</span>
									</c:when>
									<c:otherwise>
										<span title="${leafElement.name}"
											onclick="loadBillItem(${leafElement.elementid},2);">${leafElement.name}</span>
									</c:otherwise>
								</c:choose>
							</div>
						</c:forEach>
		...
		<input type="hidden" id='descriptionCondition' /> <input
			type="hidden" id='tradeCondition' /> <input type="hidden"
			id='unitCondition' />
	</div>
</div>