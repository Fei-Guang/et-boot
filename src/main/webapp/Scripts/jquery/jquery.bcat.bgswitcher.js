...
(function($, window, document, undefined) {
	...
	// Default options
	var pluginName = "bcatBGSwitcher", defaults = {
		...
		links : false
	// generate a link for each image
	};
	...
	Plugin.prototype = {
		init : function(element, options) {
			...
			instance.currentImage = this.preloadImage(element, options,
		...
		updateImage : function(element, options, instance) {
			// load an image and add it to DOM or show the new one and hide the
			// previous one
			...
			if (nextImage.length) {
				...
				nextImage = this.preloadImage(element, options,
						instance.currentIndex);
				this.swapPreloadedImages(instance.currentImage, nextImage,
						element, options);
			}
			if (options.links) {
		...
		swapPreloadedImages : function(currentImage, nextImage, element,
				options, showLoader) {
			// swap images on load
		...
		setActiveLink : function(element, options, instance) {
			...
			if (instance.linkParent.length) {
				...
				instance.linkParent.find(
						'a#' + element.id + '-link' + instance.currentIndex)
						.addClass('active');
			}
		}
	};
