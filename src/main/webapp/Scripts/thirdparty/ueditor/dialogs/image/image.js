    ...
    UploadImage.prototype = {
        ...
        getInsertList: function () {
            ...
            for (i = 0; i < this.imageList.length; i++) {
                ...
                list.push({
                    src: data.url.indexOf('http')==0 ? data.url : prefix + data.url,
                    _src: data.url.indexOf('http')==0 ? data.url : prefix + data.url,
                    title: data.title,
                    alt: data.original,
                    floatStyle: align
                });
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.width = 113;
                    img.setAttribute('src', (list[i].url.indexOf('http')==0 ? '' : urlPrefix) + list[i].url + (list[i].url.indexOf('?') == -1 ? '?noCache=':'&noCache=') + (+new Date()).toString(36) );
                    img.setAttribute('_src', list[i].url.indexOf('http')==0 ? list[i].url : urlPrefix + list[i].url);
